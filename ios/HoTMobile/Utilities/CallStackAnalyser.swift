...
//  ami-ios-base
//
//  Created by Viliam Straka on 12/12/2560 BE.
...
public class CallStackAnalyser {
    ...
    public static func classAndMethodForStackSymbol(stackSymbol: String, includeParentClass: Bool, includePackage: Bool) -> (className: String, method: String)? {
        ...
        if (components.count >= 4) {
            var swiftSymbol: SwiftSymbol = SwiftSymbol(kind: SwiftSymbol.Kind.allocator)
            do {
                swiftSymbol = try parseMangledSwiftSymbol(components[3])
            } catch {
                
            }
            
            var packageClassAndMethodStr = swiftSymbol.print(using: SymbolPrintOptions.default.union(.synthesizeSugarOnTypes))
            //var packageClassAndMethodStr = _stdlib_demangleName(components[3])
            
            let componentsPackageClass = packageClassAndMethodStr.replacingOccurrences(
                of: "\\s+",
                with: " ",
                options: .regularExpression,
                range: nil
            ...
            packageClassAndMethodStr = componentsPackageClass[0]
            
            for component in componentsPackageClass {
                if component.components(separatedBy: ".").count >= 3 {
                    packageClassAndMethodStr = component
                }
            }
            packageClassAndMethodStr = packageClassAndMethodStr.replacingOccurrences(of: "Swift.String", with: "SwiftString")
            ...
            if numberOfComponents >= 2 {
                let method = CallStackAnalyser.cleanMethod(method: packageClassAndMethod[numberOfComponents-1])
                var className = packageClassAndMethod[1]
//                if includeParentClass && numberOfComponents >= 4 {
//                    className = packageClassAndMethod[numberOfComponents-3] + "." + className
//                }
                
                if includePackage && numberOfComponents >= 3 {
                    className = packageClassAndMethod[0] + "." + className
                }
    ...
    private static func cleanMethod(method:(String)) -> String {
        if (method.count > 1) {
            ...
            if (firstChar == "(") {
                return String(method[method.index(method.startIndex, offsetBy: 1)..<method.index(method.endIndex, offsetBy: 0)])
            }
        }
        return method
    }
     ...
     */
    public static func getCallingClassAndMethodInScope(includeParentClass: Bool = true, includePackage: Bool = true) -> (className: String, method: String)? {
        let stackSymbols = Thread.callStackSymbols
        if (stackSymbols.count >= 3) {
            var useStackSymbol =  stackSymbols[2]
            if stackSymbols[2].contains("localized") || stackSymbols[2].contains("UIImageView") {
                useStackSymbol =  stackSymbols[3]
            } else if stackSymbols[2].contains("name_") {
                useStackSymbol =  stackSymbols[3]
                if stackSymbols[3].contains("name_") {
                    useStackSymbol =  stackSymbols[4]
                }
            }
            
            return CallStackAnalyser.classAndMethodForStackSymbol(stackSymbol: useStackSymbol, includeParentClass: includeParentClass, includePackage: includePackage)
        }
        return nil
    }
    ...
    /**
     ...
     - Returns: a tuple containing the (class,method) or nil if it could not be parsed
     */
    public static func getThisClassAndMethodInScope(includeParentClass: Bool = true, includePackage: Bool = true) -> (className: String, method: String)? {
        let stackSymbols = Thread.callStackSymbols
        if (stackSymbols.count >= 2) {
            return CallStackAnalyser.classAndMethodForStackSymbol(stackSymbol: stackSymbols[1], includeParentClass: includeParentClass, includePackage: includePackage)
        }
        return nil
    }
}
